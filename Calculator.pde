    ...
    for (Button b : buttons) {
      if (b.getPressed()) {
        ...
        if (input.length() == 1 && (input.charAt(0) > 47 && input.charAt(0) < 58) && a * this.b <= max / 100) {
          ...
          if (operation == 0 && a < max) a = a * 10 + Integer.parseInt(input);
          else if (this.b < max) this.b = this.b * 10 + Integer.parseInt(input);
          ...
          if (operation == 0) {
            if (Float.toString(a).length() <= 3) {
              a = 0;
            } else a = Float.parseFloat(Float.toString(a).substring(0, Float.toString(a).length() - 3));
          } else if (this.b == 0) {
            operation = 0;
          } else {
            if (Float.toString(this.b).length() <= 3) {
              this.b = 0;
            } else this.b = Float.parseFloat(Float.toString(this.b).substring(0, Float.toString(this.b).length() - 3));
          }
        }
      }
    }
